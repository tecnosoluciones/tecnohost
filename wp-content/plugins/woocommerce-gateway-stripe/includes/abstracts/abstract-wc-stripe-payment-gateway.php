...
abstract class WC_Stripe_Payment_Gateway extends WC_Payment_Gateway_CC {
	...
	public function process_response( $response, $order ) {
		...
		if ( 'yes' === $captured ) {
			...
			if ( 'pending' === $response->status ) {
				...
				$order->update_status( 'pending', sprintf( __( 'Stripe charge awaiting payment: %s.', 'woocommerce-gateway-stripe' ), $response->id ) );
			}
			...
			if ( 'succeeded' === $response->status ) {
				 ...
				 *
				 * Depending on when Stripe sends their events and how quickly it is processed by the store, the review.open webhook (which marks orders as pending)
			...
			/* translators: transaction id */
			$order->update_status( 'pending', sprintf( __( 'Stripe charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization. Refunding is unavailable until payment has been captured.', 'woocommerce-gateway-stripe' ), $response->id ) );
		}
	...
	public function send_failed_order_email( $order_id, $status_update = [] ) {
		...
		if (
			...
			in_array( $status_update['from'], [ 'pending', 'pending' ], true )
		) {
