...
class WC_Gateway_Stripe_Multibanco extends WC_Stripe_Payment_Gateway {
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		...
		if ( ! $sent_to_admin && 'stripe_multibanco' === $payment_method && $order->has_status( 'pending' ) ) {
	...
	public function process_payment( $order_id, $retry = true, $force_save_source = false ) {
		try {
			...
			$order->update_status( 'pending', __( 'Awaiting Multibanco payment', 'woocommerce-gateway-stripe' ) );
