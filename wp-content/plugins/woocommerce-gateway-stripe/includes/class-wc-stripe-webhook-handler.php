...
class WC_Stripe_Webhook_Handler extends WC_Stripe_Payment_Gateway {
	...
	public function process_webhook_payment( $notification, $retry = true ) {
		...
		try {
			...
			if ( $order->has_status( 'pending' ) && ! $is_pending_receiver ) {
				return;
			}
	 ...
	 * We want to put the order into pending and add an order note.
	 *
	 * @since 4.0.0
	 * @param object $notification
	 */
	...
	public function process_review_closed( $notification ) {
		...
		if (
			...
			if ( 'yes' === $order->get_meta( '_stripe_charge_captured' ) && in_array( $status_after_review, apply_filters( 'woocommerce_valid_order_statuses_for_payment_complete', [ 'pending', 'pending', 'failed', 'cancelled' ], $order ) ) ) {
