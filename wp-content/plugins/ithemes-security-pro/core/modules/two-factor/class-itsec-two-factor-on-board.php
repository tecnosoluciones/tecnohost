...
class ITSEC_Two_Factor_On_Board extends ITSEC_Login_Interstitial {
	...
	public function render( ITSEC_Login_Interstitial_Session $session, array $args ) {
		...
		$list = apply_filters( 'wp_sprintf_l', array(
			...
			'between'          => sprintf( __( '%1$s, %2$s', 'it-l10n-ithemes-security-pro' ), '', '' ),
			/* translators: used to join last two items in a list with more than 2 times */
			'between_last_two' => sprintf( __( '%1$s, and %2$s', 'it-l10n-ithemes-security-pro' ), '', '' ),
			/* translators: used to join items in a list with only 2 items */
			'between_only_two' => sprintf( __( '%1$s and %2$s', 'it-l10n-ithemes-security-pro' ), '', '' ),
		) );
