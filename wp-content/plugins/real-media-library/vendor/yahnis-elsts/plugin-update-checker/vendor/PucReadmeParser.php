...
if (!\class_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\PucReadmeParser', \false)) {
    /**
    ...
    class PucReadmeParser
    {
        ...
        function parse_readme_contents($file_contents)
        {
            ...
            if (!isset($final_sections['description'])) {
                $final_sections = \array_merge(array('description' => $this->filter_text($_short_description[2], \true)), $final_sections);
                $excerpt = \true;
            }
            ...
            $r = array('name' => $name, 'tags' => $tags, 'requires_at_least' => $requires_at_least, 'tested_up_to' => $tested_up_to, 'requires_php' => $requires_php, 'stable_tag' => $stable_tag, 'contributors' => $contributors, 'donate_link' => $donate_link, 'short_description' => $short_description, 'screenshots' => $final_screenshots, 'is_excerpt' => $excerpt, 'is_truncated' => $truncated, 'sections' => $final_sections, 'remaining_content' => $remaining_content, 'upgrade_notice' => $upgrade_notice);
            return $r;
        }
        ...
        {
            // whitelisted chars
            if (\function_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\user_sanitize')) {
                // bbPress native
        ...
        {
            if (\function_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\encodeit')) {
                // bbPress native
        ...
        {
            if (\function_exists('MatthiasWeb\\RealMediaLibrary\\Vendor\\decodeit')) {
                // bbPress native
