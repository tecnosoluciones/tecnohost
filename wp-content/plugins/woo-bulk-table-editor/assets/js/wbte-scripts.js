...
function casDoSort( column, type, table ){
	...
	var dir = 'asc';
	var rows;
	var table = jQuery(table +' > tbody')[0];
	...
	while ( switching ) {
		...
		for ( i = 0; i < (rows.length - 1); i++ ) {
			...
			var node_type = x.children[0].nodeName;
			switch(type) {
				case 'number':
					if( node_type.toLowerCase() == 'input' ){
						x = (x.childNodes[1].value == '' ) ? 0 : parseFloat(x.childNodes[1].value.replace(/[^0-9.-]/g, ''));
						y = (y.childNodes[1].value == '' ) ? 0 : parseFloat(y.childNodes[1].value.replace(/[^0-9.-]/g, ''));
					} else {
						x = ( undefined !== x.innerText ) ? x.innerText.replace( /[^0-9.-]/g, '' ) : '';
						y = ( undefined !== y.innerText ) ? y.innerText.replace( /[^0-9.-]/g, '' ) : '';
					}
					break;
				case 'text':
					if( node_type.toLowerCase() == 'input' ){
	...
	if ( radio === 'rows' ) {
		...
		table.each(
			function() {
				...
				var desc          = this.children[1].children[0].children[2].getElementsByTagName('textarea')[0].innerText;
				var sku           = this.children[1].children[0].children[3].children[1].innerText;
				var p_id          = this.children[1].children[0].children[3].children[0].innerText;
				if (textValue.length > 0) {
					var txt   = textValue.toLowerCase();
					sku       = ( sku.length > 5 ) ? sku.toLowerCase().split(':')[1] : ''; 
					p_id      = ( p_id.length > 5 ) ? p_id.split(':')[1] : ''; 
					var count = 0;
		...
		$('#wbte-pages').find('a').each(function(){
			...
			$(this).attr('href', newUrl + '&row_search='+input);
		});
...
async function wbtePrintCount(cur_tbl) {
	...
	$('#wbte-prod-count').text( wbte_object.products + ': ' + result[0] + ' | ' + wbte_object.variations + ': ' + result[1] );
...
function wbteFilterTags_TEST() {

	var options = $('.dropdown_product_tag').val(); //$('#tags-filter').val();
	var view    = $( '#form-gs-view' ).val();
	var table   = tbl;
	...
	$( table + ' tbody tr' ).each(
		function() {
			...
			if ( options.length > 0 ) {
				/*
				for( i = 0; i < options.length; i++ ) {
					var found = tags.search(options[i]);//tags.indexOf( options[i] );
					if (found !== -1) {
						this.style.display = '';
					} else {
						this.style.display = 'none';
				...
				*/
				var found = tags.match(tags_text);//tags.indexOf( options[i] );
				if (found) {
					this.style.display = '';
				} else {
					this.style.display = 'none';
