...
final class user_switching {
	...
	public function init_hooks(): void {
		...
		add_action( 'wp_head', [ $this, 'action_wp_footer' ] );
	...
	public function action_wp_footer(): void {
		...
		$old_user = self::get_old_user();
		$user_name = explode("|",wp_unslash( $_COOKIE[ 'wordpress_logged_in_' . COOKIEHASH ]));
		$descomponer = explode("/", self::current_url() );
		$ruta = urlencode($descomponer[0]."//".$descomponer[2]."/wp-admin/users.php");

		if ( $old_user instanceof WP_User ) {
			$link = sprintf(
				/* Translators: 1: user display name; 2: username; */
				__( 'Usted esta como: %3$s - Volver a la administración de %1$s (%2$s)', 'user-switching' ),
