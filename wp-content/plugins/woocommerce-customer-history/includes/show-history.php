...
class WCCH_Show_History {
	...
	public function render_browsing_history( $order = 0 ) {
		...
		if ( is_array( $browsing_history ) ) {
			...
			$output .= '<tr>';
			$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:left; padding:10px;">' . __( 'URL', 'woocommerce-customer-history' ) . '</th>';
			$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:left; padding:10px;">' . __( 'Timestamp', 'woocommerce-customer-history' ) . '</th>';
			$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:right; padding:10px;">' . __( 'Time on Page', 'woocommerce-customer-history' ) . '</th>';
			$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:right; padding:10px;">' . __( 'Total', 'woocommerce-customer-history' ) . '</th>';
			$output .= '</tr>';
	...
	public function render_purchase_history( $order = 0 ) {
		...
		if ( ! absint( $customer_id ) ) {
			echo '<p>' . __( 'This is a guest order not attached to any customer. Associate it with a customer account within the Order Details metabox in order to see complete purchase history.', 'woocommerce-customer-history' ) . '</p>';
			return false;
		}
		...
		$output .= '<tr>';
		$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:left; padding:10px;">' . __( 'Order Number', 'woocommerce-customer-history' ) . '</th>';
		$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:left; padding:10px;">' . __( 'Order Date', 'woocommerce-customer-history' ) . '</th>';
		$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align:left; padding:10px;">' . __( 'Order Status', 'woocommerce-customer-history' ) . '</th>';
		$output .= '<th style="background:' . $this->get_admin_color_scheme()[1] . '; color:#fff; text-align: right; padding:10px;">' . __( 'Order Total', 'woocommerce-customer-history' ) . '</th>';
		$output .= '</tr>';
		if ( ! empty( $orders ) ) {
			foreach ( $orders as $key => $purchase ) {
				...
				$output .= '<tr' . $current . '>';
				$output .= '<td style="text-align:left; padding:10px;">' . ( $key + 1 ) . '. <a href="' . admin_url( "post.php?post={$purchase_order_id}&action=edit" ) . '">' . sprintf( __( 'Order %s', 'woocommerce-customer-history' ), $purchase_order->get_order_number() ) . '</a></td>';
		...
		$output .= '<p>';
		$output .= sprintf( __( '<strong>Actual Lifetime Customer Value:</strong> %s', 'woocommerce-customer-history' ), '<span style="color:#7EB03B; font-size:1.2em; font-weight:bold;">' . wc_price( $lifetime_total ) . '</span>' );
		$output .= '</p>';
	...
	public function email_customer_history( $order = 0, $is_admin_email = false ) {
		...
		if ( true !== $is_admin_email ) {
			return;
		}

		$settings = get_option( 'woocommerce_wcch_settings', null );

		if ( isset( $settings['wcch_admin_email_browsing_enabled'] ) && 'yes' === $settings['wcch_admin_email_browsing_enabled'] ) {
			echo '<h2>' . __( 'Customer Browsing History', 'woocommerce-customer-history' ) . '</h2>';
			$this->render_browsing_history( $order );
		}

		if ( isset( $settings['wcch_admin_email_purchase_enabled'] ) && 'yes' === $settings['wcch_admin_email_purchase_enabled'] ) {
