 ...
 * MainWP Wordfence extension handler.
 */
class MainWP_Child_Wordfence { //phpcs:ignore -- NOSONAR - multi methods.
    ...
    public function __construct() {
        ...
        if ( is_plugin_active( 'wordfence/wordfence.php' ) && file_exists( plugin_dir_path( __FILE__ ) . '../../wordfence/wordfence.php' ) ) {
            require_once plugin_dir_path( __FILE__ ) . '../../wordfence/wordfence.php'; // NOSONAR - WP compatible.
     ...
     */
    public function do_reports_log( $ext = '' ) { //phpcs:ignore -- NOSONAR - ignore complex.
        if ( 'wordfence' !== $ext ) {
            return;
        }
        if ( ! $this->is_wordfence_installed ) {
        ...
        if ( true !== MainWP_Helper::instance()->check_classes_exists( array( '\wfDB' ), true ) || true !== MainWP_Helper::instance()->check_methods( '\wfDB', 'networkTable', true ) ) {
            return;
        }
    ...
    public function init_cron() {
        ...
        if ( 'twicedaily' === $sch || 'daily' === $sch || 'weekly' === $sch || 'monthly' === $sch ) {
            ...
            if ( false === $sched ) {
                wp_schedule_event( time(), $sch, 'mainwp_child_wordfence_cron_scan' );
            }
        } elseif ( false !== $sched ) {
            ...
            if ( ! empty( $mwp_action ) ) {
                switch ( $mwp_action ) { // NOSONAR - multi case.
                    ...
                    case 'check_falcon_htaccess':
                        $information = static::check_falcon_htaccess();
                        break;
                    case 'checkHtaccess':
     ...
     */
    public static function get_section_settings( $section ) { // phpcs:ignore -- NOSONAR - multi lines.
        $general_opts = array(
            'scheduleScan',
            'apiKey',
            'autoUpdate',
        ...
        switch ( $section ) {
            case static::OPTIONS_TYPE_GLOBAL:
                $options = $general_opts;
                break;
            case static::OPTIONS_TYPE_LIVE_TRAFFIC:
                $options = $traffic_opts;
                break;
            case static::OPTIONS_TYPE_FIREWALL:
                $options = $firewall_opts;
                break;
            case static::OPTIONS_TYPE_SCANNER:
                $options = $scan_opts;
                break;
            case static::OPTIONS_TYPE_DIAGNOSTICS:
                $options = $diagnostics_opts;
                break;
            case static::OPTIONS_TYPE_BLOCKING:
                $options = $blocking_opts;
                break;
            case static::OPTIONS_TYPE_ALL:
    ...
    public static function ajax_load_issues_callback() {
        ...
        return array(
            ...
            'apiKey'             => \wfConfig::get( 'apiKey' ),
            'isPaid'             => \wfConfig::get( 'isPaid' ),
            'lastscan_timestamp' => static::instance()->get_lastscan(),
            'todayAttBlocked'    => static::instance()->count_attacks_blocked( 1 ),
            'weekAttBlocked'     => static::instance()->count_attacks_blocked( 7 ),
...
SQL
     ...
     * @return array Action result.
     */
    public function bulk_operation() { //phpcs:ignore -- NOSONAR - ignore complex.
        $op = isset( $_POST['op'] ) ? sanitize_text_field( wp_unslash( $_POST['op'] ) ) : '';
        if ( 'del' === $op || 'repair' === $op ) {
            ...
            if ( $filesWorkedOn > 0 && ! empty( $errors ) ) {
     ...
     * @return array Action result.
     */
    public function save_settings_new() { //phpcs:ignore -- NOSONAR - ignore complex.
        ...
        $saving_opts = static::get_section_settings( $section );
        ...
        if ( is_array( $settings ) && count( $settings ) > 0 && count( $saving_opts ) > 0 ) {
            ...
            // if saving then validate data!
            if ( in_array( 'other_WFNet', $saving_opts ) && ! $opts['other_WFNet'] ) {
                $wfdb            = new \wfDB();
            ...
            // if saving then validate data!
            if ( in_array( 'bannedURLs', $saving_opts ) && \wfConfig::get( 'bannedURLs', false ) !== $opts['bannedURLs'] ) {
            ...
            foreach ( $opts as $key => $val ) {
                ...
                if ( in_array( $key, $saving_opts ) ) {
                    ...
                    if ( in_array( $key, static::$firewall_options_filter ) ) {
            ...
            // if saving then validate data!
            if ( in_array( 'disableCodeExecutionUploads', $saving_opts ) && isset( $opts['disableCodeExecutionUploads'] ) ) {
                try {
            ...
            // if saving then validate data!
            if ( in_array( 'email_summary_enabled', $saving_opts ) && isset( $opts['email_summary_enabled'] ) ) {
                ...
                if ( empty( $apiKey ) && empty( $existingAPIKey ) ) { // then try to get one.
                    ...
                    try {
                        ...
                        if ( $keyData['ok'] && $keyData['apiKey'] ) {
                            ...
                            throw new MainWP_Exception( "The Wordfence server's response did not contain the expected elements." );
                        }
                    } catch ( MainWP_Exception $e ) {
     ...
     * @return array Action result.
     */
    public function import_settings() { //phpcs:ignore -- NOSONAR - ignore complex.
     ...
     * @return array Action result.
     */
    private static function get_waf_data( $updated = null ) { //phpcs:ignore -- NOSONAR - ignore complex.
     ...
     * @return array Action result.
     */
    public function save_options() { //phpcs:ignore -- NOSONAR - ignore complex.
        ...
        if ( ! empty( $_POST['changes'] ) && false !== $changes ) {
            try {
                if ( is_array( $changes ) && isset( $changes['whitelistedURLParams'] ) && isset( $changes['whitelistedURLParams']['add'] ) ) {
                    ...
                    foreach ( $changes['whitelistedURLParams']['add'] as &$value ) :
                        if ( isset( $value['data'] ) ) {
            ...
            } catch ( \wfWAFStorageFileMainWP_Exception $e ) {
                return array(
     ...
     * @return array Action result.
     */
    public static function save_cache_config() { //phpcs:ignore -- NOSONAR - ignore complex.
        $noEditHtaccess = '1';
        if ( isset( $_POST['needToCheckFalconHtaccess'] ) && ! empty( $_POST['needToCheckFalconHtaccess'] ) ) {
            $checkHtaccess = static::check_falcon_htaccess();
        ...
        if ( 'falcon' === $cacheType && ! get_option( 'permalink_structure', '' ) ) {
     ...
     * @return array Action result.
     */
    public static function add_cache_exclusion() { //phpcs:ignore -- NOSONAR - ignore complex.
        ...
        \wfConfig::set( 'cacheExclusions', serialize( $ex ) ); //phpcs:ignore -- NOSONAR - complex.
        \wfCache::scheduleCacheClear();
        if ( \wfConfig::get( 'cacheType', false ) === 'falcon' && preg_match( '/^(?:uac|uaeq|cc)$/', $_POST['patternType'] ) && \wfCache::addHtaccessCode( 'add' ) ) {
            return array(
     ...
     * @return array Action result.
     */
    public function get_diagnostics() { //phpcs:ignore -- NOSONAR - ignore complex.
                                            ...
                                            if ( false !== strpos( $_SERVER[ $variable ], ',' ) ) {
                                                ...
                                                foreach ( $items as $index => $i ) {
                                                    foreach ( $trustedProxies as $proxy ) {
                                                        if ( ! empty( $proxy ) && \wfUtils::subnetContainsIP( $proxy, $i ) && $index < count( $items ) - 1 ) {
            ...
            <div class="wf-block<?php echo \wfPersistenceController::shared()->isActive( 'wf-diagnostics-wordpress-constants' ) ? ' wf-active' : ''; ?>" data-persistence-key="<?php echo esc_attr( 'wf-diagnostics-wordpress-constants' ); ?>">
                ...
                <div class="wf-block-content wf-clearfix wf-padding-no-left wf-padding-no-right">
                    <table class="wf-striped-table"<?php echo ! empty( $inEmail ) ? ' border=1' : ''; ?>>
                        <tbody>
                        <?php
                        require_once ABSPATH . 'wp-includes/version.php'; // NOSONAR - WP compatible.
                        ...
                        $debug_display = '(not set)';
                        if( defined( 'WP_DEBUG_DISPLAY' ) ){
                            $debug_display = WP_DEBUG_DISPLAY ? 'Enabled' : 'Disabled';
                        }

                        if( is_numeric( $postRevisions ) ){
                            $postRevisions_val = $postRevisions;
                        } else {
                            $postRevisions_val = $postRevisions ? 'Unlimited' : 'None';
                        }

                        $auto_update = 'Default';
                        if( defined( 'WP_AUTO_UPDATE_CORE' ) ){
                            if( is_bool( WP_AUTO_UPDATE_CORE ) ){
                                $auto_update = WP_AUTO_UPDATE_CORE ? 'Everything' : 'None';
                            } else {
                                $auto_update = WP_AUTO_UPDATE_CORE;
                            }
                        }
    ...
    public static function update_waf_rules_new() {
        ...
        return static::get_waf_data( $success );
    }
    ...
    public static function save_debugging_config() {
        $settings = isset( $_POST['settings'] ) ? wp_unslash( $_POST['settings'] ) : array();
        foreach ( static::$diagnosticParams as $param ) {
