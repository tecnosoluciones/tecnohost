...
class WOOMULTI_CURRENCY_Admin_Reports {
	...
	public function get_current_range() {
		if ( $this->current_range === null ) {
			if ( isset( $_REQUEST['_woo_multi_currency_nonce'] ) && ! wp_verify_nonce( wc_clean( wp_unslash( $_REQUEST['_woo_multi_currency_nonce'] ) ), 'wmc_admin_nonce' ) ) {
				return '7day';
			}
	...
	public function admin_enqueue_scripts() {
		...
		if ( 'woocommerce_page_wc-reports' === $screen_id ) {
			...
			$tab             = isset( $_REQUEST['tab'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['tab'] ) ) : 'orders';
			$report          = isset( $_REQUEST['report'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['report'] ) ) : '';
			$currency        = isset( $_REQUEST['wmc-currency'] ) ? strtoupper( sanitize_text_field( wp_unslash( $_REQUEST['wmc-currency'] ) ) ) : '';
			$target_currency = isset( $_REQUEST['wmc-target-currency'] ) ? strtoupper( sanitize_text_field( wp_unslash( $_REQUEST['wmc-target-currency'] ) ) ) : '';
			if ( isset( $this->currencies[ $target_currency ] ) ) {
				self::$target_rate = $this->currencies[ $target_currency ]['rate'];
			}
			$view_default = isset( $_REQUEST['wmc-view-default-currency'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['wmc-view-default-currency'] ) ) : '';
			if ( ( $tab === 'orders' && $report !== 'coupon_usage' ) || ( $tab === 'customers' && $report === 'customers' ) ) {
				...
				wp_enqueue_script( 'woocommerce-multi-currency-admin-reports', WOOMULTI_CURRENCY_JS . 'reports.js', array( 'jquery' ), WOOMULTI_CURRENCY_VERSION, false );
				wp_localize_script( 'woocommerce-multi-currency-admin-reports', 'woocommerce_multi_currency_admin_reports', array( 'currency' => $currency ? $currency : 'all-currencies' ) );
...
//						error_log( print_r( 'vvv:'.$this->total_sales, true ) );
	...
	public function admin_footer() {
		...
		$target_currency   = isset( $_REQUEST['wmc-target-currency'] ) ? strtoupper( sanitize_text_field( wp_unslash( $_REQUEST['wmc-target-currency'] ) ) ) : '';
		$view_default      = isset( $_REQUEST['wmc-view-default-currency'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['wmc-view-default-currency'] ) ) : '';
		$view_default_link = $view_default === 'yes' ? remove_query_arg( 'wmc-view-default-currency', isset( $_SERVER['REQUEST_URI'] ) ?
            sanitize_url( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '' ) : add_query_arg( array( 'wmc-view-default-currency' => 'yes' ),
                    ...
                    <select class="wmc-nav"
						...
						foreach ( $links as $code => $link ) {
                            ...
                            <option <?php selected( $target_currency, $code ) ?>
                                    value="<?php echo esc_url( remove_query_arg( array(
										'wmc-currency',
										'wmc-view-default-currency'
									), add_query_arg( array( 'wmc-target-currency' => $code ), isset( $_SERVER['REQUEST_URI'] ) ? sanitize_url( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '' ) ) ) ?>"><?php echo esc_html( "All currencies(to {$code})" ) ?></option>
							<?php
						}
						foreach ( $links as $code => $link ) {
							?>
		...
		if ( $currency && $currency !== $this->default_currency ) {
            ...
            <li class="wmc-view-default-currency-container">
                <input id="wmc-view-default-currency" type="checkbox"
                       ...
                       data-report_link="<?php esc_attr_e( remove_query_arg( array( 'wmc-target-currency' ), $view_default_link ) );// phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText ?>"><label
                        for="wmc-view-default-currency"><?php printf( '%1s(%2s)', esc_html__( 'View in default currency', 'woocommerce-multi-currency' ), esc_html( $this->default_currency ) ) ?></label>
            </li>
			<?php
		}
	}
			...
			if ( count( array_keys( $data ) ) === 3 && count( array_intersect( array_keys( $data ), array(
				...
				if ( is_array( $result ) && count( $result ) ) {
					foreach ( $result as $key => $value ) {
						...
						$val_order = wc_get_order( $order_id );
						if ( ! $val_order ) {
							continue;
						}
						$_order_currency = $val_order->get_currency();
						$wmc_order_info  = $val_order->get_meta( 'wmc_order_info', true );
						$rate            = 0;
	...
	public function woocommerce_reports_get_order_report_data_1( $result, $data ) {
		if ( isset( $data['ID'] ) && ( ! empty( $data['_order_total'] ) || ! empty( $data['_line_total'] ) ) ) {
			foreach ( $result as $data_k => $data_obj ) {
				...
				$rp_order = wc_get_order( $order_id );
				if ( $order_id && $rp_order ) {
					$_order_currency = $rp_order->get_currency();
					if ( $this->default_currency !== $_order_currency ) {
						$wmc_order_info = $rp_order->get_meta( 'wmc_order_info', true );
						$rate           = 0;
						if ( isset( $wmc_order_info[ $this->default_currency ] ) && isset( $wmc_order_info[ $this->default_currency ]['is_main'] ) && $wmc_order_info[ $this->default_currency ]['is_main'] == 1 ) {
							if ( isset( $wmc_order_info[ $_order_currency ] ) ) {
								$rate = $wmc_order_info[ $_order_currency ]['rate'];
	...
	public function wc_get_price_decimals( $decimal ) {
		...
		if ( is_admin() && ! empty( $_REQUEST['wmc-currency'] ) && $view_default !== 'yes' ) {
			$currency = strtoupper( sanitize_text_field( wp_unslash( $_REQUEST['wmc-currency'] ) ) );
	...
	public function woocommerce_currency( $woocommerce_currency ) {
		...
		if ( $pagenow === 'admin.php' && $page === 'wc-reports' ) {
			...
			if ( $currency && $view_default !== 'yes' ) {
				...
				$target_currency = strtoupper( sanitize_text_field( wp_unslash( $_REQUEST['wmc-target-currency'] ) ) );
