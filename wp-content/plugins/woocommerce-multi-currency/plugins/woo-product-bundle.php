...
class WOOMULTI_CURRENCY_Plugin_Woo_Product_Bundle {
	...
	public function __construct() {
		...
		if ( $this->settings->get_enable() && is_plugin_active( 'woo-product-bundle/wpc-product-bundles.php' ) ) {
			...
			add_action( 'woosb_before_table', array( $this, 'woosb_before_table' ) );
			add_filter( 'woosb_item_price_before_set', array( $this, 'woosb_item_price_before_set' ), 10, 2 );
	...
	public function woosb_item_price_before_set( $_price, $cart_item ) {
		$_price = wmc_revert_price( $_price );
			...
			if ( ! $product->is_fixed_price() ) {
				...
				if ( $product->is_optional() ) {
					...
					if ( $discount_amount ) {
						...
						$min_price *= (float) ( 100 - floatval( $discount_percentage ) ) / 100;
					}
					...
					if ( $discount_amount ) {
						...
						$min_price *= (float) ( 100 - floatval( $discount_percentage ) ) / 100;
						$max_price *= (float) ( 100 - floatval( $discount_percentage ) ) / 100;
					}
					...
					foreach ( $items as $item ) {
						...
						if ( $_product ) {
							...
							if ( $discount_percentage ) {
								...
								$price_sale += apply_filters( 'woosb_item_price_add_to_cart', $_price * ( 100 - floatval( $discount_percentage ) ) / 100, $item );
							}
						}
					}
