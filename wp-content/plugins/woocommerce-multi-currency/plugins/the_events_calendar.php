...
/**
 * Class WOOMULTI_CURRENCY_Plugin_The_Events_Calendar
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
//			wp_reset_postdata();
			if ( count( $prs ) ) {
				...
				if ( is_array( $cost ) ) {
					...
					foreach ( $cost as $c ) {
						if ( $with_currency_symbol ) {
							$return[] = ! empty( $c ) ? $this->wc_price( wmc_get_price( $c ) ) : '';
						} else {
							$return[] = ! empty( $c ) ? wmc_get_price( $c ) : '';
						}
					}
					...
					if ( $with_currency_symbol ) {
						return ! empty( $cost ) ? $this->wc_price( wmc_get_price( $cost ) ) : '';
					} else {
						return ! empty( $cost ) ? wmc_get_price( $cost ) : '';
					}
				}
