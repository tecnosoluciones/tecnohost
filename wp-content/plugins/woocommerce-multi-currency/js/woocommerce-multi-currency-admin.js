jQuery(document).ready(function ($) {
    ...
    $('select[name="woo_multi_currency_params[billing_shipping_currency]"]').on('change', function () {
        ...
        let $only_shipping = $('input[name="woo_multi_currency_params[only_shipping_country]"]').closest('tr');
        if ($(this).val() === '0') {
            $sync_currency.fadeOut(200);
            $only_shipping.fadeOut(200);
        } else {
            $sync_currency.fadeIn(200);
            if ($(this).val() === '2') {
                $only_shipping.fadeIn(200);
            } else {
                $only_shipping.fadeOut(200);
            }
        }
    ...
    $('select[name="woo_multi_currency_params[cache_compatible]"]').on('change', function () {
        let val = $(this).val();
        if ('1' === val) {
            $('.wmc-ajax-filter-price-row').show();
        } else {
            $('.wmc-ajax-filter-price-row').hide();
        }
    }).trigger('change');
    $('input[name="woo_multi_currency_params[currency_by_payment_method_immediate]"]').on('change', function () {
        if ($(this).prop('checked')) {
            $('.wmc-currency-by-payment-row').fadeIn(200);
        } else {
            $('.wmc-currency-by-payment-row').fadeOut(200);
        }
    }).trigger('change');
});
