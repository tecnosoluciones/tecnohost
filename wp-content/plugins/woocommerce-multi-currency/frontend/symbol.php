...
class WOOMULTI_CURRENCY_Frontend_Symbol {
	...
	public function my_account_page_orders( $order ) {
		...
		/* translators: %1$s: order total, %2$s: order item count*/
		printf( _n( '%1$s for %2$s item', '%1$s for %2$s items', $item_count, 'woocommerce-multi-currency' ), wp_kses_post( $total ), wp_kses_post( $item_count ) );// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
