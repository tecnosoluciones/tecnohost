...
/**
 * Class WOOMULTI_CURRENCY_Frontend_Filter_Price
 */
class WOOMULTI_CURRENCY_Frontend_Filter_Price {
	protected $settings;
	public $min_price;
	...
	public function wp_enqueue_scripts() {
		if ( is_shop() || is_post_type_archive( 'product' ) ) {
			wp_enqueue_script( 'woocommerce-multi-currency-filter-price', WOOMULTI_CURRENCY_JS . 'filter-price.js', array( 'jquery' ), WOOMULTI_CURRENCY_VERSION, false );
		}
	}
	...
	public function woocommerce_price_filter_widget_step( $step ) {
		$new_step = intval( wmc_get_price( $step, false, true ) );
		if ( $new_step > 1 ) {
			$this->step = $new_step;
			$step       = $new_step;
		} else {
	...
	public function reset_price( $args, $wp_query ) {
		...
		$current_currency = $this->settings->get_current_currency();
		$selected_currencies = $this->settings->get_list_currencies();
		$rate                = $current_currency && isset( $selected_currencies[ $current_currency ] ) ?
			(float) $selected_currencies[ $current_currency ]['rate'] : wmc_get_price( 1, $current_currency );
