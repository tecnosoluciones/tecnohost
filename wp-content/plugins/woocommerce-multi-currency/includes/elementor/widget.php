...
class WOOMULTI_CURRENCY_Elementor_Widget extends Widget_Base {
	...
	public function enqueue() {
		wp_register_style( 'wmc-elementor-style', false, [], WOOMULTI_CURRENCY_VERSION );
		wp_enqueue_style( 'wmc-elementor-style' );
		$css = ".wmc-multi-currency-icon{
				display:block;height:28px;background-image: url('" . WOOMULTI_CURRENCY_IMAGES . "icon_elementor.svg');
	...
	public function get_title() {
		return esc_html__( 'Multi Currency', 'woocommerce-multi-currency' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'general',
			[
				'label' => esc_html__( 'General', 'woocommerce-multi-currency' ),
		...
		$this->add_control(
			'layout',
			[
				'label'       => esc_html__( 'Select switcher layout', 'woocommerce-multi-currency' ),
		...
		$this->add_control(
			'flag_size',
			[
				'label'     => esc_html__( 'Flag size', 'woocommerce-multi-currency' ),
		...
		$this->add_control(
			'direction',
			[
				'label'       => esc_html__( 'Direction', 'woocommerce-multi-currency' ),
				'type'        => \Elementor\Controls_Manager::SELECT,
				'label_block' => true,
				'options'     => [
					'bottom' => esc_html__( 'Bottom', 'woocommerce-multi-currency' ),
					'top'    => esc_html__( 'Top', 'woocommerce-multi-currency' ),
				],
				'condition'   => [
					'layout' => [ 'plain_vertical' ]
				]
			]
		);
	...
	protected function render() {
		...
		$direction = $settings['direction'] ? 'direction=' . $settings['direction'] : '';
		echo do_shortcode( "[woo_multi_currency{$layout} $flag_size $direction]" );
	}
	...
	public function render_plain_content() {
		...
		$flag_size = $settings['flag_size'] ? 'flag_size=' . $settings['flag_size'] : '';
		$direction = $settings['direction'] ? 'direction=' . $settings['direction'] : '';
		$shortcode = "[woo_multi_currency{$layout} $flag_size $direction]";
