<?php
/**
 * Subscription Product Enhancer Options.
 */
...
extract( $meta_values );
?>
<div class="options_group show_if_subscription hidden _enr_subscription_fields_wrapper">
	<p class="form-field _enr_enable_seperate_shipping_cycle_field">
		...
		<input type="checkbox" class="checkbox" name="_enr_enable_seperate_shipping_cycle" id="_enr_enable_seperate_shipping_cycle" value="yes" <?php checked( 'yes', $enable_seperate_shipping_cycle ); ?>/>
	...
	<p class="form-field _enr_enable_seperate_shipping_cycle_for_old_subscriptions_field">
		<label for="_enr_enable_seperate_shipping_cycle_for_old_subscriptions"><?php esc_html_e( 'Separate shipping cycle for old subscriptions', 'enhancer-for-woocommerce-subscriptions' ); ?></label>
		<input type="checkbox" class="checkbox" name="_enr_enable_seperate_shipping_cycle_for_old_subscriptions" id="_enr_enable_seperate_shipping_cycle_for_old_subscriptions" value="yes" <?php checked( 'yes', $enable_seperate_shipping_cycle_for_old_subscriptions ); ?>/>
	...
	<p class="form-field _enr_shipping_frequency_field">
		...
		<span class="wrap">
			...
			<input type="number" class="wc_input_price short" name="_enr_shipping_period_interval" id="_enr_shipping_period_interval" style="margin-right: 10px;" value="<?php echo esc_attr( $shipping_period_interval ); ?>" min="0"/>
			...
			<select id="_enr_shipping_period" name="_enr_shipping_period">
				<?php foreach ( wcs_get_subscription_period_strings() as $value => $label ) { ?>
					<option value="<?php echo esc_attr( $value ); ?>" <?php selected( $value, $shipping_period, true ); ?>><?php echo esc_html( $label ); ?></option>
				<?php } ?>
			</select>
		</span>
	...
	<p class="form-field _enr_shipping_frequency_sync_date_field">
		<label for="_enr_shipping_frequency_sync_date"><?php esc_html_e( 'Synchronise shipping frequency', 'enhancer-for-woocommerce-subscriptions' ); ?></label>
		<span class="wrap">
			<select id="_enr_shipping_frequency_sync_date_day" name="_enr_shipping_frequency_sync_date_day">
			...
			<select id="_enr_shipping_frequency_sync_date_week" name="_enr_shipping_frequency_sync_date_week">
	...
	<?php if ( 'yes' === get_option( ENR_PREFIX . 'allow_cancelling', 'yes' ) ) { ?>
		...
		<p class="form-field _enr_allow_cancelling_after_field">
			...
			<input type="number" class="wc_input_price short" name="_enr_allow_cancelling_after" id="_enr_allow_cancelling_after" value="<?php echo esc_attr( metadata_exists( 'post', $post->ID, ENR_PREFIX . 'allow_cancelling_after' ) ? $allow_cancelling_after : '0'  ); ?>"/>
			<span class="description"><?php esc_html_e( 'day(s) from the subscription start date', 'enhancer-for-woocommerce-subscriptions' ); ?></span>
		</p>
		<p class="form-field _enr_allow_cancelling_after_due_field">
			<label for="_enr_allow_cancelling_after_due"><?php esc_html_e( 'Allow cancelling after each renewal', 'enhancer-for-woocommerce-subscriptions' ); ?>
				<span class="woocommerce-help-tip" data-tip="<?php esc_attr_e( 'Set 0 to allow subscribers to cancel immediately. If empty, customers will not be allowed to cancel.', 'enhancer-for-woocommerce-subscriptions' ); ?>"></span>
			</label>
			<input type="number" class="wc_input_price short" name="_enr_allow_cancelling_after_due" id="_enr_allow_cancelling_after_due" value="<?php echo esc_attr( metadata_exists( 'post', $post->ID, ENR_PREFIX . 'allow_cancelling_after_due' ) ? $allow_cancelling_after_due : '0'  ); ?>"/>
		...
		<p class="form-field _enr_allow_cancelling_before_due_field">
			...
			<input type="number" class="wc_input_price short" name="_enr_allow_cancelling_before_due" id="_enr_allow_cancelling_before_due" value="<?php echo esc_attr( metadata_exists( 'post', $post->ID, ENR_PREFIX . 'allow_cancelling_before_due' ) ? $allow_cancelling_before_due : '0'  ); ?>"/>
	...
	<?php } ?>
	<p class="form-field _enr_allow_price_update_for_old_subscriptions_field">
		<label for="_enr_allow_price_update_for_old_subscriptions"><?php esc_html_e( 'Subscription price update for old subscriptions', 'enhancer-for-woocommerce-subscriptions' ); ?></label>
		<select class="select short _enr_allow_price_update_for_old_subscriptions" id="_enr_allow_price_update_for_old_subscriptions" name="_enr_allow_price_update_for_old_subscriptions">
	...
	<p class="form-field _enr_subscription_price_for_old_subscriptions_field">
		<label for="_enr_subscription_price_for_old_subscriptions"><?php esc_html_e( 'Subscription price for old subscriptions', 'enhancer-for-woocommerce-subscriptions' ); ?>
		...
		<select class="select short _enr_subscription_price_for_old_subscriptions" id="_enr_subscription_price_for_old_subscriptions" name="_enr_subscription_price_for_old_subscriptions">
	...
	<p class="form-field _enr_notify_subscription_price_update_before_field">
		...
		<input type="number" class="wc_input_price short" name="_enr_notify_subscription_price_update_before" id="_enr_notify_subscription_price_update_before" value="<?php echo esc_attr( $notify_subscription_price_update_before ); ?>"/>
	...
	<p class="form-field _enr_exclude_reminder_emails_field">
		<label for="_enr_exclude_reminder_emails"><?php esc_html_e( 'Disable reminder emails', 'enhancer-for-woocommerce-subscriptions' ); ?>
		...
		<select class="wc-enhanced-select" name="_enr_exclude_reminder_emails[]" id="_enr_exclude_reminder_emails" multiple="multiple">
