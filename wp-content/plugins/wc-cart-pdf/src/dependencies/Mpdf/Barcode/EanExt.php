 ...
 */
class EanExt extends \WCCartPDF\Mpdf\Barcode\AbstractBarcode implements \WCCartPDF\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code, $length = 5)
	{
		...
		if ($length == 2) {
			...
			throw new \WCCartPDF\Mpdf\Barcode\BarcodeException(sprintf('Invalid EAN barcode value "%s"', $code));
		}
