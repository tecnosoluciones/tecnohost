 ...
 */
class Imb extends \WCCartPDF\Mpdf\Barcode\AbstractBarcode implements \WCCartPDF\Mpdf\Barcode\BarcodeInterface
{
	...
	public function __construct($code, $xDim, $gapWidth, $daft)
	{
		if (!function_exists('bcadd')) {
			throw new \WCCartPDF\Mpdf\Barcode\BarcodeException('IMB barcodes require bcmath extension to be loaded.');
		}
	...
	private function init($code, $gapWidth, $daft)
	{
		...
		switch (strlen($routingCode)) {
			...
			case 11:
				...
				break;
			default:
				throw new \WCCartPDF\Mpdf\Barcode\BarcodeException(sprintf('Invalid MSI routing code "%s"', $routingCode));
		}
