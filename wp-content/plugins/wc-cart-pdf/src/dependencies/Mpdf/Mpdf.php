...
class Mpdf implements \WCCartPDF\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \WCCartPDF\Mpdf\QrCode\QrCode
	 */
	private $qrcode;
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] === 'barcode') {
				...
				if (in_array($objattr['btype'], $barcodeTypes, true)) {
					...
					if (!class_exists('WCCartPDF\Mpdf\QrCode\QrCode') || !class_exists('WCCartPDF\Mpdf\QrCode\Output\Mpdf')) {
						throw new \WCCartPDF\Mpdf\MpdfException('WCCartPDF\Mpdf\QrCode package was not found. Install the package from Packagist with "composer require mpdf/qrcode"');
					}
