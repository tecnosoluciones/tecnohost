...
namespace WCCartPDF\Mpdf\Pdf;

use WCCartPDF\Mpdf\Pdf\Protection\UniqidGenerator;
...
class Protection
{
	...
	/**
	 * @var \WCCartPDF\Mpdf\Pdf\Protection\UniqidGenerator
	 */
	...
	public function setProtection($permissions = [], $user_pass = '', $owner_pass = null, $length = 40)
	{
		...
		if ($length === 128) {
			...
			throw new \WCCartPDF\Mpdf\MpdfException('PDF protection only allows lenghts of 40 or 128');
		}
	...
	private function getProtectionBitsFromOptions($permissions)
	{
		...
		foreach ($permissions as $permission) {
			if (!isset($this->options[$permission])) {
				throw new \WCCartPDF\Mpdf\MpdfException(sprintf('Invalid permission type "%s"', $permission));
			}
