...
$isDomain = ($addon['name'] == 'Dominio') ? true : (($addon['name'] == 'Nombre del Dominio') ? true : false);
?>

<div class="form-row form-row-wide wc-pao-addon-wrap wc-pao-addon-<?php echo esc_attr( sanitize_title( $field_name ) ); ?> <?php echo $isDomain ? "wc-pao-addon-dominio" : ""; ?>">
	<input
		type="text"
		class="input-text wc-pao-addon-field wc-pao-addon-custom-text <?php echo $isDomain ? "wc-pao-addon-dominio" : ""; ?>"
	...
	if($isDomain){
	 ?>
	 	<input type="hidden" id="domain-field" name="domain-field"/>
	 <?php
	    }
	 ?>
</div>
