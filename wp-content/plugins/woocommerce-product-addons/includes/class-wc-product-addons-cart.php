		...
		if ( ! empty( $cart_item_data['addons'] ) && apply_filters( 'woocommerce_product_addons_adjust_price', true, $cart_item_data ) ) {
			...
			foreach ( $cart_item_data['addons'] as $addon ) {
				...
				switch ( $price_type ) {
					...
					case 'quantity_based':
					    $recurrencia = 1;
					   if($cart_item_data['variation']['attribute_pa_plan']){
					       $recurrencia = explode('-', $cart_item_data['variation']['attribute_pa_plan']);
					       switch ($recurrencia[0]){
                        		case  "trimestral":
                                $recurrencia = 3;
                                break;
                        		case  "semestral":
						...
						$price         += (float) $recurrencia*$addon_price;
						$regular_price += (float) $recurrencia*$addon_price;
						$sale_price    += (float) $recurrencia*$addon_price;
						break;	
					/*Modificación*/
					default:
