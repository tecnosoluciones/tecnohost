...
class PluginUpdaterPro {
	...
	/**
	 * Check if there's a new version before gathering further information and showing any update notification
	 *
	 * @return bool
	 */
	public function decide_to_call_repo() {
		$check_latest_version_response_code = false;

		$response = wp_safe_remote_get( 'http://drm6aghn7w1h8.cloudfront.net/_wpacu-pro-latest-version.txt' );

		if ( ! is_wp_error( $response ) ) {
			$check_latest_version_response_code = $response['response']['code'];
		}

		$latest_version_check_response_successful = ! is_wp_error( $check_latest_version_response_code ) &&
            $check_latest_version_response_code >= 200 &&
            $check_latest_version_response_code < 300;

		if ($latest_version_check_response_successful && isset($response['body']) && $response['body']) {
			$latest_version = $response['body'];

			if (version_compare($latest_version, WPACU_PRO_PLUGIN_VERSION, '>')) {
				return true;
			}
		}
