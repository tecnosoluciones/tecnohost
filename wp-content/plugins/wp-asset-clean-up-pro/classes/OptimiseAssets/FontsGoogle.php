...
class FontsGoogle
{
	...
	public function init()
	{
		...
		add_action('wp_footer', static function() {
			if ( Plugin::preventAnyFrontendOptimization() || Main::isTestModeActiveAndVisitorNonAdmin() || Main::instance()->settings['google_fonts_remove'] ) {
				return;
			}
	...
	public function resourceHints($urls, $relationType)
	{
		// don't apply any changes if not in the front-end view (e.g. Dashboard view)
		// or test mode is enabled and a guest user is accessing the page
		if (is_admin() || Main::isTestModeActiveAndVisitorNonAdmin() || Plugin::preventAnyFrontendOptimization()) {
			return $urls;
		}
