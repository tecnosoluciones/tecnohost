...
class CleanUp
{
	...
	public static function removeHtmlComments($htmlSource, $ignoreExceptions = false)
	{
		...
		$fromTheCache = false; // default
        $comments = ObjectCache::wpacu_cache_get('wpacu_all_html_comments');
        ...
        $allCommentsToSave = array();

		foreach ($comments as $comment) {
			if ( ! $fromTheCache ) {
                $entireComment = '<!--' . $comment->nodeValue . '-->';
            } else {
                $entireComment = $comment;
            }
            ...
            $allCommentsToSave[] = $entireComment;
        ...
        ObjectCache::wpacu_cache_set('wpacu_all_html_comments', $allCommentsToSave);
