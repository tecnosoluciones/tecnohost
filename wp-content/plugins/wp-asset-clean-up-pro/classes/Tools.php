	...
	public function importDone()
    {
	    ...
	    foreach ($this->data['import_done_list'] as $importedKey) {
            ...
            if ($importedKey === 'global_data') {
                ...
                if ( ( ! empty($globalDataValues['styles']['positions']) )
                  || ( ! empty($globalDataValues['scripts']['positions']) ) ) {
	                $importedMessage .= '<li>' . esc_html__( 'Any CSS/JS updated positions (to &lt;HEAD&gt; or &lt;BODY&gt;)', 'wp-asset-clean-up-pro' ) . '</li>';
                }

	            if ( ( ! empty($globalDataValues['styles']['preloads']) )
                  || ( ! empty($globalDataValues['scripts']['preloads']) ) ) {
		            $importedMessage .= '<li>' . esc_html__( 'Any CSS/JS preloading (rel="preload")', 'wp-asset-clean-up-pro' ) . '</li>';
	            }

	            if ( ! empty($globalDataValues['scripts']['everywhere']) ) {
                ...
                if (    ( ! empty($globalDataValues['plugins']) )
                     || ( ! empty($globalDataValues['plugins_dash']) ) )  {
