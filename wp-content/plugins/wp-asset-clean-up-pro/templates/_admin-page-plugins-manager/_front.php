...
<div data-wpacu-sub-page-area="<?php echo $data['wpacu_sub_page']; ?>"
     class="wpacu-wrap <?php if ( ! $data['plugins_manager_front_enable'] ) { ?>wpacu-area-disabled<?php } ?>"
	...
	<form method="post" action="" class="wpacu_settings_form">
		...
		foreach ($data['active_plugins'] as $pluginData) {
			...
			$data['tax_group_list'] = PluginsManagerPro::generatePublicTaxonomyListForDd(esc_attr($data['plugin_path']));
			// [/Load/Unload via taxonomy]

			// [Load/Unload via archive type]
			$data['archive_group_list'] = PluginsManagerPro::generateArchivePageTypesList();
			// [/Load/Unload via archive type]
