...
class WC_Gateway_BACS extends WC_Payment_Gateway {
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		 ...
		 */
		if ( ! $sent_to_admin && 'bacs' === $order->get_payment_method() && $order->has_status( apply_filters( 'woocommerce_bacs_email_instructions_order_status', 'pending', $order ) ) ) {
			if ( $this->instructions ) {
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			...
			$order->update_status( apply_filters( 'woocommerce_bacs_process_payment_order_status', 'pending', $order ), __( 'Awaiting BACS payment', 'woocommerce' ) );
		} else {
