...
class WC_Gateway_Cheque extends WC_Payment_Gateway {
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		 ...
		 */
		if ( $this->instructions && ! $sent_to_admin && 'cheque' === $order->get_payment_method() && $order->has_status( apply_filters( 'woocommerce_cheque_email_instructions_order_status', 'pending', $order ) ) ) {
	...
	public function process_payment( $order_id ) {
		...
		if ( $order->get_total() > 0 ) {
			...
			$order->update_status( apply_filters( 'woocommerce_cheque_process_payment_order_status', 'pending', $order ), _x( 'Awaiting check payment', 'Check payment method', 'woocommerce' ) );
		} else {
