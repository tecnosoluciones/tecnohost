...
class Module extends Module_Base {
	...
	public static function enqueue_scripts() {
        ...
        if ( ! wp_script_is( 'pa-waypoints', 'enqueued' ) ) {
			wp_enqueue_script( 'pa-waypoints' );
		}
	...
	public function register_controls( $element ) {
		...
		$repeater->add_control(
			...
			array(
				'label'       => __( 'Scale to Container Width/Height', 'premium-addons-pro' ),
				'type'        => Controls_Manager::SWITCHER,
				'description' => __( 'Enable this to scale the element until its width/height is equal to the parent container width/height.', 'premium-addons-pro' ),
				'condition'   => array(
		...
		$element->add_control(
			...
			array(
				'raw'             => __( 'This means that the animation will start and complete once the viewport\'s top hits the top of the container.' . $refresh_button, 'premium-addons-pro' ),
				'type'            => Controls_Manager::RAW_HTML,
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'condition'       => array(
					'premium_mscroll_switcher'     => 'yes',
		...
		$element->add_control(
			...
			array(
				...
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'top'    => __( 'Top of Viewport Hits The Container', 'premium-addons-pro' ),
					'center' => __( 'Center of Viewport Hits The Container', 'premium-addons-pro' ),
		...
		$element->add_control(
			...
			array(
				...
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'section' => __( 'Top of the parent container', 'premium-addons-pro' ),
	...
	private function register_responsive_controls( $element ) {
		...
		$element->add_control(
			...
			array(
				...
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'window' => __( 'Container Height Larger Than Window Height', 'premium-addons-pro' ),
					'custom' => __( 'Container Height Larger Than Specfic Height', 'premium-addons-pro' ),
				),
				'default'     => 'window',
				'label_block' => true,
				'condition'   => array(
		...
		if ( 'yes' === $mscroll_switcher && isset( $settings['premium_mscroll_repeater'] ) ) {
			...
			if ( 'widget' === $type && \Elementor\Plugin::instance()->editor->is_edit_mode() ) {
				...
				$element->add_render_attribute(
					'mscroll' . $id,
					array(
						'id'           => 'premium-mscroll-' . $id,
						'data-mscroll' => wp_json_encode( $mscroll_settings ),
					)
				);
				?>
				<div <?php echo wp_kses_post( $element->get_render_attribute_string( 'mscroll' . $id ) ); ?>></div>
				<?php
			}
		}
	}
	...
	public function check_script_enqueue( $element ) {
		...
		if ( ! empty( $settings[ 'premium_mscroll_switcher' ] ) ) {
