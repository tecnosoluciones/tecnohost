...
class Premium_Twitter_Feed extends Widget_Base {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			'consumer_secret',
			array(
				'label'       => __( 'Consumer Secret', 'premium-addons-pro' ),
				'type'        => Controls_Manager::TEXT,
				'label_block' => true,
			)
		);
	...
	protected function render() {
		...
		if ( empty( $settings['username'] ) || empty( $settings['consumer_key'] ) || empty( $settings['consumer_secret'] ) ) {

			?>
			<div class="premium-error-notice">
				<?php echo esc_html( __( 'Please fill the required fields: Consumer Key, Consumer Secret and Username', 'premium-addons-pro' ) ); ?>
			</div>
			<?php
			return;
		}

		$credentials = base64_encode( $settings['consumer_key'] . ':' . $settings['consumer_secret'] ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode

		$id = $this->get_id();

		$transient_name = sprintf( 'papro_tweets_%s_%s', $settings['username'], $id );

		$tweets_response = get_transient( $transient_name );

		if ( false === $tweets_response ) {
			...
			if ( ! empty( $tweets_response['response'] ) && 200 === $tweets_response['response']['code'] ) {
				...
				$tweets_response = json_decode( wp_remote_retrieve_body( $tweets_response ), true );

				$expire_time = Helper_Functions::transient_expire( $settings['reload'] );

				set_transient( $transient_name, $tweets_response, $expire_time );
