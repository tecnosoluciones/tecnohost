...
class Premium_Hscroll extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium horizontal scroll', 'slider', 'full', 'scene' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$temp_repeater->add_control(
			...
			array(
				...
				'options' => array(
					...
					'id'       => __( 'Container ID', 'premium-addons-pro' ),
				),
				'default' => 'id',
			)
		);
		...
		$temp_repeater->add_control(
			'section_id',
			array(
				'label'     => __( 'Container ID', 'premium-addons-pro' ),
				'type'      => Controls_Manager::TEXT,
				'dynamic'   => array( 'active' => true ),
				'condition' => array(
					'template_type' => 'id',
		...
		$temp_repeater->add_control(
			'hide_section',
			array(
				'label'              => __( 'Hide Container On', 'premium-addons-pro' ),
				'type'               => Controls_Manager::SELECT2,
				'multiple'           => true,
				'label_block'        => true,
				'options'            => array_merge(
		...
		$this->add_responsive_control(
			...
			array(
				...
				'type'               => Controls_Manager::SLIDER,
				'description'        => __( 'Set value in pixels for the scroll distance after last slide before scroll down to next container', 'premium-addons-pro' ),
				'range'              => array(
					'px' => array(
						'min' => 0,
						'max' => 300,
				...
				<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'scroller_wrap' ) ); ?>>
					<div class="premium-hscroll-sections-wrap" data-scroll-opacity="<?php echo esc_attr( $opacity ); ?>">
						...
						foreach ( $templates as $index => $section ) :
							...
							} elseif ( ! empty( $section['anchor_id'] ) ) {
									$this->add_render_attribute(
										'section_' . $index,
										array(
