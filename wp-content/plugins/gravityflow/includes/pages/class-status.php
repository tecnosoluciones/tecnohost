...
class Gravity_Flow_Status_Table extends WP_List_Table {
	...
	public function get_columns() {
		...
		if ( ! empty( $args['form-id'] ) && ! is_array( $args['form-id'] ) ) {
			$legacy_mode = ! empty( $this->legacy );
			$columns     = Gravity_Flow_Common::get_field_columns( $columns, rgar( $args, 'form-id' ), $this->field_ids, $legacy_mode, false );
        }
