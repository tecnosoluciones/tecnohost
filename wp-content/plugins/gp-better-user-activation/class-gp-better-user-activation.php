...
class GP_Better_User_Activation extends GP_Perk {
	...
	public function init() {
		...
		// i18n
		load_plugin_textdomain( 'gp-better-user-activation', false, basename( dirname( __file__ ) ) . '/languages/' );
	...
	public function show_content( $content ) {
		...
		/**
		 * Avada (the theme/plugin that packages Fusion Template Builder) calls `the_content` filter on multiple content
		 * blocks on the same page. WordPress will still call the `the_content` filter on the default page content. Let's
		 * check if this function is being called from a chain that originates in Avada's render_content() function and
		 * ignore those requests.
		 */
		if ( class_exists( 'Fusion_Template_Builder' ) ) {
			$functions = debug_backtrace( DEBUG_BACKTRACE_IGNORE_ARGS );
			foreach ( $functions as $func ) {
				if ( rgar( $func, 'class' ) === 'Fusion_Template_Builder' && $func['function'] === 'render_content' ) {
					return $content;
				}
			}
		}
