 ...
 */
abstract class WpmfGoogle_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param WpmfGoogle_Client $client  The current Google client
   */
  public function __construct(WpmfGoogle_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('WpmfGoogle_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('WpmfGoogle_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('WpmfGoogle_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'WpmfGoogle_Logger_Abstract',
        'allow_newlines'
    );
  }
   ...
   * @throws WpmfGoogle_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new WpmfGoogle_Logger_Exception(
