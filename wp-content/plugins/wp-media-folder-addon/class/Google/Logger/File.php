 ...
 */
class WpmfGoogle_Logger_File extends WpmfGoogle_Logger_Abstract
{
  /**
  ...
  public function __construct(WpmfGoogle_Client $client)
  {
    ...
    $file = $client->getClassConfig('WpmfGoogle_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('WpmfGoogle_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('WpmfGoogle_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new WpmfGoogle_Logger_Exception('File pointer is no longer available');
    }
