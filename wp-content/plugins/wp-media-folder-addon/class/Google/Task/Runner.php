...
class WpmfGoogle_Task_Runner
{
  ...
  /**
   * @var WpmfGoogle_Client $client The current API client.
   */
  private $client;
   ...
   *
   * @param WpmfGoogle_Client $client The current API client
   * @param string $name The name of the current task (used for logging)
   * @param callable $action The task to run and possibly retry
   * @param array $arguments The task arguments
   * @throws WpmfGoogle_Task_Exception when misconfigured
   */
  public function __construct(
      WpmfGoogle_Client $client,
      $name,
      $action,
      array $arguments = array()
  ) {
    $config = (array) $client->getClassConfig('WpmfGoogle_Task_Runner');

    if (isset($config['initial_delay'])) {
      if ($config['initial_delay'] < 0) {
        throw new WpmfGoogle_Task_Exception(
   ...
   *
   * @return mixed
   * @throws WpmfGoogle_Task_Retryable on failure when no retries are available.
   */
  public function run()
  {
    while ($this->attempt()) {
      ...
      } catch (WpmfGoogle_Task_Retryable $exception) {
