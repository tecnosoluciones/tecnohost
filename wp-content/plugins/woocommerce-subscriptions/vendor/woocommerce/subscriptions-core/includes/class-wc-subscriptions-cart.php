...
class WC_Subscriptions_Cart {
	...
	public static function calculate_subscription_totals( $total, $cart ) {
        ...
        //cambio por JD 06/09/2022
        $contador_sub = 0;
		// Group the subscription items by their cart item key based on billing schedule
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			if ( WC_Subscriptions_Product::is_subscription( $cart_item['data'] ) ) {
                //cambio por JD 06/09/2022
                $subscription_groups[ self::get_recurring_cart_key( $cart_item,"",$contador_sub ) ][] = $cart_item_key;
			}
            //cambio por JD 06/09/2022
            $contador_sub++;
		}
	 ...
	 * @since 1.0.0 - Migrated from WooCommerce Subscriptions v2.0
	 */
	public static function get_recurring_cart_key( $cart_item, $renewal_time = '', $contador ) {
        ...
        switch ( $interval ) {
            ...
            default:
                //cambio por JD 06/09/2022
                $cart_key .= sprintf( 'every_%dth_%s', $product_id.$contador, $period );
                break;
        }

		if ( $length > 0 ) {
			$cart_key .= '_for_';
            $cart_key .= sprintf( '%d_%s', $product_id, $period );
			if ( $length > 1 ) {
				$cart_key .= 's';
			}
		}
