...
if ( ! class_exists( 'YITH_Licence' ) ) {
	...
	abstract class YITH_Licence {
		 ...
		 * @var string
		 */
		protected $api_uri = 'https://google-licence.yithemes.com/api/';
		...
		public function __construct() {
			...
			if ( $is_debug_enabled ) {
				$this->api_uri = defined( 'YIT_LICENCE_DEBUG_LOCALHOST' ) ? YIT_LICENCE_DEBUG_LOCALHOST : 'https://google-staging-licenceyithemes.kinsta.cloud/api/';
		...
		public function admin_enqueue_scripts() {
			...
			$license_and_download_url = 'https://google-yithemes.com/my-account/recent-downloads/';
				...
				*/
				$response = 200;
						$license = array(
							...
							*/
							'email'                => 'google@yithemes.com',
							'licence_key'          => 'KB2119A0-JRK5-YA1H-OA8D-AFMBDH10E5H5',
		...
		protected function do_request( $url, $body = array(), $method = 'POST' ) {
			...
			if ( is_wp_error( $response ) || ( isset( $response['response'] ) && isset( $response['response']['code'] ) && floor( $response['response']['code'] / 100 ) >= 4 ) ) {
				$alternative_url = esc_url_raw( add_query_arg( $body, 'https://google-casper.yithemes.com/wc-api/software-api/' ) );
