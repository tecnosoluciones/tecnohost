...
class ThemeUpgrader extends Theme_Upgrader {
    ...
    public function install_package($args = array()) {
        ...
        if (is_dir($destination) && file_exists("$destination/style.css")) {
            ...
            $this->strings['installing_package'] = __nb('Upgrading the theme&#8230;');
            $this->strings['remove_old'] = __nb('Backing up the old version of the theme&#8230;');
        }
    ...
    public function clear_destination($destination) {
        ...
        $backup_result = $this->createBackup($destination);
        ...
        $this->skin->error($backup_result);
        $this->skin->feedback(__nb('Moving the old version of the theme to a new directory&#8230;'));

        $data = $this->getThemeInfo("$destination/style.css");
        ...
        if (is_dir("$directory/$new_name")) {
            ...
            $this->skin->error(__nb('Unable to find a new directory name to move the old version of the theme to. No backup will be created.'));
        } else {
            $result = $wp_filesystem->move($destination, "$directory/$new_name");

            if ($result) {
                /* translators: 1: new theme directory name */
                $this->skin->feedback(__nb('Moved the old version of the theme to a new theme directory named %1$s. This directory should be backed up and removed from the site.', "<code>$new_name</code>"));
            } else {
                $this->skin->error(__nb('Unable to move the old version of the theme to a new directory. No backup will be created.'));
            }
        }
    ...
    private function createBackup($directory) {
        return NobunaBackup::CreateBackup($directory, $this->nobunaProduct, $this->skin, TRUE);
    }
    ...
    private function getThemeInfo($file) {
        return Package::GetThemeInfo($file);
    }

    private function get_random_characters($min_length, $max_length) {
        return Str::GetRandomCharacters($min_length, $max_length);
    }

    function set_minimum_memory_limit($new_memory_limit) {
        System::SetMinimumMemoryLimit($new_memory_limit);
    }
