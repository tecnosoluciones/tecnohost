...
jQuery(function(){
    ...
    if(URLactual == "/dominios/" || URLactual == "/hosting/" || URLactual == "/hosting-avanzado-cpanel-wordpress/" || URLactual == "/hosting-basico-directadmin/" || URLactual == "/hosting-basico-cpanel/" || URLactual == "/hosting-basico/") {
        jQuery("section#panl_rev").hide();
        jQuery("div#pl_hosp a").removeAttr("href");
        jQuery("div#pl_rev a").removeAttr("href");
        //jQuery("div#pl_hosp a").attr("id","active");
...
jQuery(window).on( "load", function() {
    var body_elemen = jQuery("body");

    body_elemen.on("click","input.selec_plan", function(a,u){
        id_producto = jQuery(this).val();
        btn_principal = jQuery("a#re_cam");
        jQuery("input.cantid").val("");
        btn_principal.attr("data-select", "");
        jQuery("input.selec_plan").prop('checked', false);
        jQuery(this).prop('checked', true);
        btn_principal.removeAttr("style");
        btn_principal.attr("data-select", id_producto);
        jQuery("input[name='quantity["+id_producto+"]']").val(1);
        mostrar_plan_nuevo(id_producto);
    });

    body_elemen.on("click","a#re_cam", function(a,u){
        jQuery(".selec_plan").each(function(){
            if(jQuery(this).prop("checked") === true){
                id_producto = jQuery(this).val();
            }
        });
        jQuery("#loader").removeAttr("style");
        jQuery("button[value='"+id_producto+"']").click();
    });

    jQuery("a#re_cam").attr("style","opacity: .5 !important; cursor: not-allowed; pointer-events: none;");
    jQuery("a#re_cam").removeAttr("href");
});

function mostrar_plan_contratado(){
    jQuery("#loader").removeAttr("style");
    jQuery.ajax({
        url : dcms_vars.ajaxurl,
        type: 'post',
        data: {
            action : 'producto_contratado',
            id_producto : jQuery("form.cart").attr("data-id"),
        },
        success: function(resultado){
            jQuery("div.plan_actual div.elementor-widget-wrap").prepend(resultado);
            jQuery("#loader").css("display","none");
            jQuery('input[id*="nombre-del-dominio"]').val(jQuery("input#dominio").val());
        }
    });
}

function mostrar_plan_nuevo(id_producto){
    jQuery("#loader").removeAttr("style");
    jQuery("div.plan_nuevo div.elementor-widget-wrap").html("");
    jQuery.ajax({
        url : dcms_vars.ajaxurl,
        type: 'post',
        data: {
        ...
        success: function(resultado){
            jQuery("div.plan_nuevo div.elementor-widget-wrap").append(resultado);
            jQuery("#loader").css("display","none");
        }
    });
}

function dominio_actual(){
    jQuery("#loader").removeAttr("style");
    jQuery.ajax({
        url : dcms_vars.ajaxurl,
        type: 'post',
        data: {
            action : 'obtener_dominio',
            id_subscription : window.location.search.split("=")[1].split("&")[0],
        },
        async: false,
        success: function(resultado){
            jQuery("input#addon-"+jQuery("input[name='add-to-cart']").val()+"-nombre-del-dominio-0").val(resultado);
            jQuery("input.wc-pao-addon-dominio").val(resultado);
            agregar_do = "<div id='dom_act'> <div class='tex-dom'>Dominio asociado:</div><div id='dom_te'>"+resultado+"</div></div>";
            /*jQuery("div.elementor-widget-premium-addon-title.premium-title-icon-row").css("margin-bottom","0");*/
            jQuery("div.elementor-widget-premium-addon-title.premium-title-icon-row  div.premium-title-container").append(agregar_do);
        }
    });
}

function obtener_variation(){
    jQuery.ajax({
        url : dcms_vars.ajaxurl,
        type: 'post',
        data: {
            action : 'obtener_variation',
            id_subscription : window.location.search.split("=")[1].split("&")[0],
        },
        async: false,
        success: function(resultado){
            JSON.parse(jQuery(".variations_form").attr("data-product_variations")).forEach(function(element){
                id_variation = element.variation_id;
                if(id_variation == resultado){
                    selector_option = element.attributes.attribute_pa_periodicidad;
                    elemen_option = jQuery("select#pa_periodicidad option[value='"+selector_option+"']");
                    mensaje = elemen_option.text();
                    jQuery("select#pa_periodicidad").attr("dom-actual",selector_option);
                    elemen_option.text(mensaje+" Dominio Actual");
                    elemen_option.attr("selected", true).change();
                    agregar_ed = "<div id='pl_act'> <div class='tex-pal'>Forma de pago actual:</div> "+element.price_html+"</div>";
                    jQuery("div.elementor-widget-premium-addon-title.premium-title-icon-row  div.premium-title-container").append(agregar_ed);
...
function validar_moneda_orden(moneda_orden, moneda_definida){
    if(moneda_orden !== moneda_definida){
        alert("Cambiamos la moneda de la plataforma para igualarla a la de tu servicio.");
        jQuery(".wmc-select-currency-js option[value='"+moneda_orden+"']").attr("selected", true).change();
    }
    else{
