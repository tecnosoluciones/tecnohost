 ...
 */
class Get_domain_item extends AutomateWoo\Variable {
    ...
    public function get_value( $order, $parameters ) {
        foreach ($order->get_items() as $item_id => $item) {
            $name = wc_get_order_item_meta($item->get_id(), 'Nombre del Dominio', true);
            if($name == ""){
                $name = wc_get_order_item_meta($item->get_id(), 'Dominio', true);
            }
            break;
        }
        return $name;
