...
global $product, $post;
$cambio = 0;
if($_REQUEST['switch-subscription']){
...
foreach($grouped_products AS $key => $value){
    if($product_id == $value->get_id()){
        $eliminar = $key;
        break;
    }
}
unset($grouped_products[$eliminar]);

do_action( 'woocommerce_before_add_to_cart_form' ); ?>

<form class="cart grouped_form" data-id="<? echo $product_id; ?>" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
		...
		<tbody>
            ...
            if($cambio == 1){
                foreach($grouped_products AS $key => $value){
                    ...
                    echo '
                        <input style="display:none;" type="number" class="input-text cantid text" step="1" min="1" max="" name="quantity['.$product_id.']" value="" title="Cantidad" size="4" placeholder="" inputmode="numeric" autocomplete="off">
                        <input style="display:none;" type="text" class="input-text wc-pao-addon-field wc-pao-addon-custom-text" data-raw-price="0" data-price="" name="addon-'.$product_id.'-0" id="addon-'.$product_id.'-0" data-price-type="flat_fee" value="'.$dominio.'" maxlength="255" required="">';
                }
            }
